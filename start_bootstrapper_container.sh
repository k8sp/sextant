...
for ((i=0;i<len;i++)); do
  ...
  DOCKER_IMAGE=`grep "${DOCKER_IMAGES[i]}:" $1/config/cluster-desc.yml | awk '{print $2}' | sed 's/ //g' | sed -e 's/^"//' -e 's/"$//'`
  DOCKER_TAR_FILE=$1/$(echo ${DOCKER_IMAGE}.tar | sed "s/:/_/g" |awk -F'/' '{print $2}')
