tart_bootstrapper_container.sh load docker image from bsroot and
...
for ((i=0;i<len;i++)); do
  ...
  #DOCKER_TAR_FILE=`echo $1/${DOCKER_IMAGE}.tar | sed "s/:/_/g" |awk -F'/' '{print $2}'`
  DOCKER_TAR_FILE=`echo $1/${DOCKER_IMAGE}.tar | sed "s/:/_/g"`
