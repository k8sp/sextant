...
func main() {
	...
	if *validate == true {
		...
		err := validation(*clusterDescFile, *ccTemplateFile, *caKey, *caCrt, *dir)
		if err != nil {
			fmt.Print("Failed: \n" + err.Error())
			os.Exit(1)
		}
...
func validation(clusterDescFile string, ccTemplateFile string, caKey, caCrt, dir string) error {
	clusterDesc, err := ioutil.ReadFile(clusterDescFile)
	candy.Must(err)

	ccTemplate, err := ioutil.ReadFile(ccTemplateFile)
	candy.Must(err)
